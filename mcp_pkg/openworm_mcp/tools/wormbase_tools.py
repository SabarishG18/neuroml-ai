...
def _resolve_gene_id(gene_id: str) -> str:
    """Resolve a common gene name to a WBGene ID using the ParaSite API.

    If gene_id already looks like a WBGene ID (starts with 'WBGene'), returns
    it unchanged. Otherwise queries the ParaSite lookup endpoint.
    ...
    try:
        ...
        if wb_id and wb_id.startswith("WBGene"):
            logger.info(f"Resolved gene name '{gene_id}' → {wb_id}")
            return wb_id
    except Exception as e:
        logger.warning(f"Could not resolve gene name '{gene_id}': {e}")
